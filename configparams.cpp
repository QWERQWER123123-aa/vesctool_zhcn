...
QString ConfigParams::getLongName(const QString &name)
{
    ...
    if (mParams.contains(name)) {
        retVal = QCoreApplication::translate("ConfigParams", mParams[name].longName.toUtf8().constData());
    } else {
        qWarning() << name << "not found";
    }
...
QString ConfigParams::getDescription(const QString &name)
{
    ...
    if (mParams.contains(name)) {
        retVal = QCoreApplication::translate("ConfigParams", mParams[name].description.toUtf8().constData());
    } else {
        qWarning() << name << "not found";
    }
...
QStringList ConfigParams::getParamEnumNames(const QString &name)
{
    ...
    if (mParams.contains(name)) {
        ...
        if (p.type == CFG_T_ENUM || p.type == CFG_T_BITFIELD) {
            for (const auto &enumName : p.enumNames) {
                retVal.append(QCoreApplication::translate("ConfigParams", enumName.toUtf8().constData()));
            }
        } else {
            qWarning() << name << "wrong type";
        }
