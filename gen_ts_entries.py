# gen_ts_entries.py
# Save this file in f:\VESC\vesc_tool-master\vesc_tool-master and run:
#   python gen_ts_entries.py
...
for xml_file in config_dir.rglob("*.xml"):
    ...
    for elem in root.iter("longName"):
        if elem.text and elem.text != "none":
            translations[elem.text] = TR_DICT.get(elem.text, "")
...
print('    <context>')
print('        <name>ConfigParams</name>')
count = 0
for source, trans in sorted(translations.items()):
    if not trans:
...
print(f"\n<!-- Total: {len(translations)} messages, {count} unfinished -->")