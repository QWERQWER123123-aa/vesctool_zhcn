...
void HelpDialog::showHelp(QWidget *parent, ConfigParams *params, QString name, bool modal)
{
    ...
    if (param) {
        HelpDialog *h = new HelpDialog(QCoreApplication::translate("ConfigParams", param->longName.toUtf8().constData()),
                                       QCoreApplication::translate("ConfigParams", param->description.toUtf8().constData()),
                                       parent);
        if (modal) {
            h->exec();
        } else {
